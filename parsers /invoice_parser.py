...
def extract_invoice_metadata_intelligent(pdf_file) -> Dict[str, Any]:
    ...
    try:
        ...
        # 3. Aggressive Name Extraction
        cust_match = re.search(r"(?:Bill\s*To|Customer\s*Name|To)[\s\:]+([A-Za-z0-9\s\.\,\&\-]+?)(?:\s+(?:Invoice|Date|Amount|Terms|Ship|Receipt|Total|[\$\d]))", full_text_clean, re.IGNORECASE)
        if cust_match:
...
def parse_zoho_summary_pdf_bulletproof(pdf_file) -> List[ZohoRecord]:
    ...
    try:
        ...
        # CORE FIX: The regex now uses (.+?) to act as an absolute wildcard vacuum, bypassing the slash error.
        pattern = re.compile(r"(INV-[A-Za-z0-9\-]+)\s+(.+?)\s+\$?([0-9,]+\.\d{2})")
        ...
        for match in matches:
            ...
            # Now that it vacuumed up everything, we delete any dates that got dragged along
            clean_name = re.sub(r'\b\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4}\b', '', raw_name)
            clean_name = re.sub(r'\s+', ' ', clean_name).strip()
            
            gross = clean_numeric_value(gross_str)
            
            # Look ahead for the fee
